...
import { najemnikFields } from "../schema/fields";

export default function NajemnikModule() {
  ...
  return (
    <div>
      <div className="card">
        <h2>Seznam nájemníků</h2>
        <button className="btn primary" onClick={() => setShowForm(true)}>+ Přidat nájemníka</button>
      ...
      {showForm && (
        <div className="card">
          <h2>Přidat nájemníka</h2>
          <form onSubmit={handleSubmit}>
            <div className="form-row">
              <div>
                <label>Typ nájemníka</label>
                <select name="typNajemnika" value={form.typNajemnika || ""} onChange={handleChange} required>
            ...
            {najemnikFields(form.typNajemnika).map((field) => (
              <div key={field.name}>
                <label>
                  {field.label}
                  {field.required && " *"}
