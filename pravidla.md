...
## Společná databáze subjektů

- Všechny subjekty (osoba, OSVČ, firma, spolek/skupina, státní organizace, zástupce atd.) jsou vedeny ve **společné databázi/tabulce**.
- Typ subjektu je určující (pole `typ_subjektu` – enum).

## Vazby mezi moduly/entitami

- **Všechny vazby (pronajímatel-nemovitost, nájemník-jednotka, smlouva-nájemník, atd.) jsou realizovány přes unikátní ID** (např. `pronajimatel_id`, `jednotka_id`), NIKDY přes text/název.
- Formuláře, tabulky i importy/exporty musí pole s vazbou vždy řešit přes výběr z existující entity (číselník), nikoliv volným textem.

## Dynamické pole podle typu subjektu

- Každý formulář i tabulka musí dynamicky zobrazovat pole podle zvoleného typu subjektu (např. IČO/DIČ jen pro firmy, datum narození jen pro osobu atd.).

## Importy, exporty, datové modely a API

- Vždy obsahují pole `typ_subjektu` a ID všech vazeb (např. `jednotka_id`, `pronajimatel_id`, …).
- Ukázkové JSONy vždy zobrazují vazby přes ID.

## Pravidlo nemazání obsahu

- **Žádný obsah v dokumentaci ani datech se nesmí mazat!**  
  Používej pouze přeškrtnutí (strikethrough), přesuny, komentáře a rozšiřování.  
  Historii a kontext je nutné zachovat!

## Refaktoring, rozšíření, checklisty

- Každý návrh změny, rozšíření či refaktoringu musí zachovat historii (přeškrtnout staré, přidat nové, nikdy neodstraňovat).
- Každý modul musí na začátku obsahovat reference na ostatní související moduly.
- Checklist každé sekce musí obsahovat bod „Vazby na ostatní entity přes ID“.
- Všechny nové ikony a akce musí být zapsány do centrálního katalogu.

---

> **Dodržováním těchto pravidel zajistíme, že veškeré moduly a data budou snadno propojitelné, udržíme historii a kontext a aplikace bude rozšiřitelná bez ztráty informací.**
